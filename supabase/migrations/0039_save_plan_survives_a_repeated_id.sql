-- A save that claims one row twice saves what it can, rather than nothing.
--
-- save_plan upserts by id, and Postgres refuses a statement that touches the
-- same row twice: "ON CONFLICT DO UPDATE command cannot affect row a second
-- time". A day starts and ends at the hotel, so a client matching stops to
-- stored rows by what they are can hand the same id to both -- and the whole
-- plan is then refused, silently, because the drag that caused it had already
-- painted.
--
-- The client counts repeats now. This is the other half: a repeated id here is
-- taken as a stop that has no row yet and given a new one, so the plan is
-- written either way.
