...
import { afford } from './budget.ts';
import type { LatLng, Mode } from './input.ts';

/**
 * One journey, routed.
 *
 * Lives here rather than in the route function because two callers need it:
 * the client asking about a leg it is drawing, and the refiner working through
 * a whole plan's worth of estimates after the fact. One implementation means
 * one cache, one field mask, and one place where money is spent.
 */
 ...
 *
 * Charged before Google is asked, not after: a request refused once the money
 * is spent is not a budget. A cached answer costs nothing and is not charged.
 */
export async function routeLeg(
	...
	// to be told so.
	if (key(from) === key(to)) return { route: null, error: 'no_journey' };
	...
	// One route is one billed element.
	if (!(await afford(db, traveller, 1))) return { route: null, error: 'budget' };
	...
	if (!res.ok) {
		...
		return { route: null, error: `routes_${res.status}` };
	}
